...
"""
Análisis Mejorado de Indicadores Económicos - Versión 2
...
Utiliza los datos reales y limpios generados por el script limpiar_datos_v2.py.
"""
...
def cargar_datos():
    ...
    if not os.path.exists('indicadores_economicos_clean_v2.csv'):
        print("No se encontraron datos limpios v2. Ejecutando script de limpieza...")
        from limpiar_datos_v2 import limpiar_datos_indicadores_v2
        limpiar_datos_indicadores_v2()
...
def analizar_estacionariedad(df):
    ...
    # Ajustar diseño
    plt.tight_layout()
    plt.savefig('series_temporales_analisis_v2.png')
    plt.close()
    ...
    # Ajustar diseño
    plt.tight_layout()
    plt.savefig('series_temporales_diferenciadas_v2.png')
    plt.close()
...
def analisis_de_correlacion(df):
    ...
    plt.tight_layout()
    plt.savefig('correlacion_v2.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('correlacion_retardos_v2.png')
    plt.close()
...
def regresion_polinomica_mejorada(df):
    ...
    for x_var, y_var, x_label, y_label, color, lag in relaciones:
        ...
        if mejor_modelo is not None:
            ...
            # Guardar figura
            file_name = f'regresion_polinomica_v2_{y_var}_{x_var}_lag{lag}.png'
    ...
    resultados_df.to_csv('resultados_regresiones_polinomicas_v2.csv', index=False)
...
def modelar_arima(df):
    ...
    # Tamaño del conjunto de prueba (últimos 12 meses o 20% de los datos)
    test_size = min(12, int(len(df) * 0.2))
    test_size = max(test_size, 1)  # Asegurar al menos un punto para test
    ...
    for columna in df.columns:
        ...
        try:
            ...
            plt.legend()
            plt.grid(True, alpha=0.3)
            plt.savefig(f'arima_prediccion_v2_{columna}.png')
            plt.close()
...
def generar_informe(df, resultados_estacionariedad, resultados_arima, resultados_polinomicos, resultados_correlaciones):
    ...
    with open('informe_analisis_v2.md', 'w') as f:
        f.write('# Informe de Análisis Mejorado de Indicadores Económicos (Datos Reales)\n\n')
        ...
        f.write(str(df.corr()))
        f.write('\n```\n\n')
        f.write('![Matriz de Correlación](correlacion_v2.png)\n\n')
        
        # Correlación con retardos
        f.write('### Correlación con Retardos Temporales\n\n')
        f.write('![Correlación con Retardos](correlacion_retardos_v2.png)\n\n')
        ...
        f.write('![Series Temporales](series_temporales_analisis_v2.png)\n\n')
        f.write('### Series Temporales Diferenciadas\n\n')
        f.write('![Series Diferenciadas](series_temporales_diferenciadas_v2.png)\n\n')
        ...
        for columna, resultados in resultados_arima.items():
            ...
            f.write(f'![Predicción ARIMA](arima_prediccion_v2_{columna}.png)\n\n')
        ...
        f.write('1. **Correlaciones Mejoradas**: Los datos reales muestran correlaciones significativas entre los indicadores económicos, destacando la fuerte correlación negativa entre tipo de cambio y tasa de interés.\n\n')
    ...
    print("Informe generado: informe_analisis_v2.md")

def main():
    """Función principal."""
    print("Iniciando análisis mejorado de indicadores económicos con datos reales...")
    ...
    print("\nAnálisis mejorado v2 completado.")
